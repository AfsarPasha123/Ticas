  ...
  try {
    ...
    if (tags) {
      ...
      if (Array.isArray(tags) && tags.length > 0) {
        ...
        const validTagIds = tagIds.filter((id): id is number => id !== undefined);
...
export const searchProductsByTags = async (
  ...
  try {
    ...
    if (tags) {
      ...
      let tagArray: string[] = [];
      if (typeof tags === 'string') {
        tagArray = tags.split(',').map((tag: string) => tag.trim().toLowerCase());
      }
      ...
      tagIds = tagRecords.map(tag => tag.id).filter((id): id is number => id !== undefined);
    }
    ...
    if (category && typeof category === 'string') {
      ...
      if (categoryRecord && categoryRecord.tags) {
        ...
        const categoryTagIds = categoryRecord.tags.map(tag => tag.id).filter((id): id is number => id !== undefined);
...
export const searchProductsByCategory = async (
  ...
  try {
    ...
    const tagIds = (categoryRecord.tags?.map(tag => tag.id) || []).filter((id): id is number => id !== undefined);
