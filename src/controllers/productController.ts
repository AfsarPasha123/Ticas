...
export const createProduct = async (
  inputProductData: ProductAttributes, 
  req?: Request, 
  res?: Response
) => {
  ...
  const validDonationStatuses: Array<ProductAttributes['donation_status']> = ['in_donation', 'donated'];
...
export const getAllProducts = async (req: Request, res: Response) => {
  try {
    ...
    if (donation_status) {
      ...
      if (!validDonationStatuses.includes(donation_status as string)) {
    ...
    const productsWithUrls = await Promise.all(
      products.map(async (product) => {
        ...
        if (productJson.primary_image_url) {
          productJson.primary_image_url = await getSignedDownloadUrl(
            productJson.primary_image_url
          );
        }
        return productJson;
      })
...
export const getProductsByDonationStatus = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
