...
export const moveToDonation = async (req, res) => {
    ...
    try {
        await Product.update({ donation_status: 'in_donation' }, { where: { product_id: productIds } });
...
export const donateProducts = async (req, res) => {
    ...
    try {
        await Product.update({ donation_status: 'donated' }, { where: { product_id: productIds } });
        return res.status(HTTP_STATUS.OK).json({
            status: RESPONSE_TYPES.SUCCESS,
...
export const getInDonationProducts = async (req, res) => {
    try {
